    ...
    if (!existentColaborador) {
      const options = {
        ...
        data: {
          ...
          email: colaborador.email,
          document: colaborador.documento.numero,
          type: colaborador.tipo,

          default_bank_account: {
            holder_name: colaborador.nome,
            holder_type: colaborador.tipo,
            holder_document: colaborador.documento.numero,
            bank: colaborador.contaBancaria.banco,
            branch_number: colaborador.contaBancaria.agencia,
            account_number: colaborador.contaBancaria.numero,
            account_check_digit: colaborador.contaBancaria.dv,
            type: colaborador.contaBancaria.tipoConta,
          },
        }
      };
...
router.get('/salao/:salaoId', async (req, res) => {
    try {
        ...
        for (let vinculo of salaoColaboradores) {
           ...
           // Filtrar elementos nulos do array especialidades
          const filteredEspecialidades = especialidades.filter((especialidade) => especialidade.servicoId);

          listaColaboradores.push({
            ...vinculo._doc,
            especialidades: filteredEspecialidades.map(
