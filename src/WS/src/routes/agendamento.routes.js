...
router.delete('/deletarPorAgendamento/:agendamentoId', async (req, res) => {
  try {
    const { agendamentoId } = req.params;

    // Encontre e remova o agendamento com base no agendamentoId
    const resultado = await Agendamento.findByIdAndDelete(agendamentoId);

    if (!resultado) {
      return res.json({ error: true, message: 'Agendamento não encontrado' });
    }

    res.json({ error: false, message: 'Agendamento deletado com sucesso' });
  } catch (err) {
...
router.get('/filtrarPorCliente/:clienteId', async (req, res) => {
  try {
    const { clienteId } = req.params;

    // Obtém a data atual usando o Moment.js
    const currentDate = moment().startOf('day');

    // Filtra os agendamentos para o cliente especificado e data maior ou igual à atual
    const agendamentos = await Agendamento.find({
      clienteId,
    ...
    // Mapeia os agendamentos para incluir apenas as informações desejadas
    const agendamentosResumidos = agendamentos.map(agendamento => ({
    ...
    res.json({ error: false, agendamentos: agendamentosResumidos });
  } catch (err) {
    res.json({ error: true, message: err.message });
  }
});
    ...
    for (let i = 0; i <= 365 && agenda.length <= 7; i++) {
      ...
      if (espacosValidos.length > 0) {
        ...
        for (let colaboradorId of Object.keys(todosHorariosDia)) {
          ...
          // agenda.push(lastDay.format('YYYY-MM-DD'));
      }
